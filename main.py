...
for epitech_account in epitech_accounts:
    ...
    # config
    DATA_PATH = epitech_account.get('google_user_data_path')
    PROFILE_DIR = epitech_account.get('google_profile')
    driver = create_driver(DATA_PATH, PROFILE_DIR)
    bypass_anti_ddos(driver)
    EPITECH_COOKIE = epitech_account.get('epitech_cookie')
    EPITECH_LOGIN = get_epitech_login(EPITECH_COOKIE, driver)
    ...
    if CALENDAR_ID_EVENTS is not None:
        ...
        my_events = get_my_epitech_events(EPITECH_COOKIE, driver, start=monday_of_current_week)
    ...
    if CALENDAR_ID_TIMELINE is not None:
        ...
        my_projects = get_my_epitech_projects(EPITECH_COOKIE, driver, start=monday_of_current_week)
    ...
    if CALENDAR_ID_TEACHING_TEAM is not None:
        ...
        my_google_assistant_events = get_my_assistant_events(EPITECH_COOKIE, EPITECH_LOGIN, driver, start=monday_of_current_week)
    ...
    if CALENDAR_ID_OTHER_CALENDARS is not None:
        ...
        my_other_calendars_events = get_my_epitech_other_calendars_events(EPITECH_COOKIE, driver, start=monday_of_current_week)
